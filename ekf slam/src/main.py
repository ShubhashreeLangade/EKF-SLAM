...
from visualize import (
    plot_trajectory_with_imu,
    plot_trajectory,
    plot_slam_vs_landmarks,
    plot_ekf_slam_2d
)
...
for t in range(N):
    ...
    # Landmark Update
    mu, Sigma = update_landmarks_lidar(
        mu, Sigma,
        lidar_measurements,
        R
    )
...
slam_landmarks = np.array([
    mu[6+3*i:6+3*i+3] for i in range(M)
])
...
plot_ekf_slam_2d(mu_history, slam_landmarks, dataset_number)
